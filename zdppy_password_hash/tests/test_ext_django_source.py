"""
test zdppy_password_hash.ext.django against django source tests
"""
#=============================================================================
# imports
#=============================================================================
...
# site
# pkg
from zdppy_password_hash.utils.compat import suppress_cause
from zdppy_password_hash.ext.django.utils import DJANGO_VERSION, DjangoTranslator, _PasslibHasherWrapper
# tests
from zdppy_password_hash.tests.utils import TestCase, TEST_MODE
    ...
    if source_path:
        ...
        if TEST_MODE("full"):
            ...
            sys.stderr.write("\nWARNING: $PASSLIB_TESTS_DJANGO_SOURCE_PATH is not set; "
                             "can't run Django's own unittests against zdppy_password_hash.ext.django\n")
...
# but with zdppy_password_hash monkeypatched in.
#----------------------------------------------------------------------
if test_hashers_mod:
    ...
    class HashersTest(test_hashers_mod.TestUtilsHashPass, _ExtensionSupport):
        """
        Run django's hasher unittests against zdppy_password_hash's extension
        ...
        # port patchAttr() helper method from zdppy_password_hash.tests.utils.TestCase
        ...
        def setUp(self):
            #---------------------------------------------------------
            # install zdppy_password_hash.ext.django adapter, and get context
            #---------------------------------------------------------
            ...
            #---------------------------------------------------------
            from zdppy_password_hash.hash import django_des_crypt
            self.patchAttr(django_des_crypt, "use_duplicate_salt", False)

            #---------------------------------------------------------
            # install receiver to update scheme list if test changes settings
            #---------------------------------------------------------
            django_to_zdppy_password_hash_name = DjangoTranslator().django_to_zdppy_password_hash_name
            ...
            def update_schemes(**kwds):
                ...
                schemes = [
                    django_to_zdppy_password_hash_name(import_string(hash_path)())
            ...
            def update_rounds():
                """
                sync django hasher config -> zdppy_password_hash hashers
                """
                for handler in context.schemes(resolve=True):
                    if 'rounds' not in handler.setting_kwds:
                        continue
                    hasher = adapter.zdppy_password_hash_to_django(handler)
            ...
            # sync whenever adapter tries to resolve zdppy_password_hash hasher
            self.patchAttr(adapter, "django_to_zdppy_password_hash", update_wrapper, wrap=True)
        ...
        _OMIT = lambda self: self.skipTest("omitted by zdppy_password_hash")
