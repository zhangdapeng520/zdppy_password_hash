"""test zdppy_password_hash.ext.django"""
#=============================================================================
# imports
#=============================================================================
# core
...
# site
# pkg
from zdppy_password_hash import apps as _apps, exc, registry
from zdppy_password_hash.apps import django10_context, django14_context, django16_context
from zdppy_password_hash.context import CryptContext
from zdppy_password_hash.ext.django.utils import (
    DJANGO_VERSION, MIN_DJANGO_VERSION, DjangoTranslator, quirks,
)
from zdppy_password_hash.utils.compat import iteritems, get_method_function, u
from zdppy_password_hash.utils.decor import memoized_property
# tests
from zdppy_password_hash.tests.utils import TestCase, TEST_MODE, handler_derived_from
from zdppy_password_hash.tests.test_handlers import get_handler_case
# local
__all__ = [
    "DjangoBehaviorTest",
...
def _modify_django_config(kwds, sha_rounds=None):
    ...
    # update defaults
    kwds.update(
        # TODO: push this to zdppy_password_hash.apps django contexts
        deprecated="auto",
    )
...
#      or maybe add a "get_django_context(django_version)" helper to zdppy_password_hash.apps?
...
#----------------------------------------------------
from zdppy_password_hash.hash import django_pbkdf2_sha256
sample_hashes = dict(
...
class _ExtensionSupport(object):
    ...
    def assert_unpatched(self):
        ...
        mod = sys.modules.get("zdppy_password_hash.ext.django.models")
        ...
        for obj, attr, source, patched in self._iter_patch_candidates():
            if patched:
                ...
                self.assertFalse(source.startswith("zdppy_password_hash."),
    ...
    def assert_patched(self, context=None):
        ...
        for obj, attr, source, patched in self._iter_patch_candidates():
            if patched:
                self.assertTrue(source == "zdppy_password_hash.ext.django.utils",
    ...
    def load_extension(self, check=True, **kwds):
        ...
        import zdppy_password_hash.ext.django.models
        if check:
    ...
    def unload_extension(self):
        ...
        if mod:
            ...
            del sys.modules["zdppy_password_hash.ext.django.models"]
...
class DjangoBehaviorTest(_ExtensionTest):
    ...
    running the ExtensionBehaviorTest subclass below verifies "zdppy_password_hash.ext.django"
    ...
    #: tracks whether tests should assume "zdppy_password_hash.ext.django" monkeypatch is applied.
    ...
    # and run against the zdppy_password_hash extension, to verify it matches those assumptions.
    #
    ...
    def test_extension_config(self):
        ...
        from zdppy_password_hash.ext.django.models import password_context
    ...
    def _do_test_available_scheme(self, scheme):
        ...
        :param scheme: *zdppy_password_hash* name of hasher (e.g. "django_pbkdf2_sha256")
        """
        log = self.getLogger()
        ctx = self.context
        patched = self.patched
        ...
        #-------------------------------------------------------
        alg = DjangoTranslator().zdppy_password_hash_to_django_name(scheme)
        ...
        name = DjangoTranslator().django_to_zdppy_password_hash_name(identify_hasher(hash).algorithm)
...
class ExtensionBehaviorTest(DjangoBehaviorTest):
    """
    test that "zdppy_password_hash.ext.django" conforms to behavioral assertions in DjangoBehaviorTest
    """
...
class DjangoExtensionTest(_ExtensionTest):
    """
    test the ``zdppy_password_hash.ext.django`` plugin
    """
    #===================================================================
    # class attrs
    #===================================================================

    descriptionPrefix = "zdppy_password_hash.ext.django plugin"
    ...
    def test_02_handler_wrapper(self):
        ...
        zdppy_password_hash_to_django = DjangoTranslator().zdppy_password_hash_to_django

        # should return native django hasher if available
        if DJANGO_VERSION > (1, 10):
            self.assertRaises(ValueError, zdppy_password_hash_to_django, "hex_md5")
        else:
            hasher = zdppy_password_hash_to_django("hex_md5")
        ...
        hasher = zdppy_password_hash_to_django("django_bcrypt")
        self.assertIsInstance(hasher, hashers.BCryptPasswordHasher)

        # otherwise should return wrapper
        from zdppy_password_hash.hash import sha256_crypt
        hasher = zdppy_password_hash_to_django("sha256_crypt")
        self.assertEqual(hasher.algorithm, "zdppy_password_hash_sha256_crypt")
        ...
        self.assertRaises(KeyError, zdppy_password_hash_to_django, "does_not_exist")
    ...
    def test_13_config_defaults(self):
        ...
        from zdppy_password_hash.ext.django.utils import PASSLIB_DEFAULT
        default = CryptContext.from_string(PASSLIB_DEFAULT)
        self.load_extension()
        self.assert_patched(PASSLIB_DEFAULT)

        # check default preset
        self.load_extension(PASSLIB_CONTEXT="zdppy_password_hash-default", check=False)
        self.assert_patched(PASSLIB_DEFAULT)
    ...
    def test_14_config_invalid(self):
        ...
        self.assertRaises(TypeError, __import__, 'zdppy_password_hash.ext.django.models')

        self.unload_extension()
        update_settings(PASSLIB_CONFIG="missing-preset", PASSLIB_CONTEXT=UNSET)
        self.assertRaises(ValueError, __import__, 'zdppy_password_hash.ext.django.models')
