"""helpers for zdppy_password_hash unittests"""
#=============================================================================
# imports
#=============================================================================
...
import threading
import time
from zdppy_password_hash.exc import PasslibHashWarning, PasslibConfigWarning
from zdppy_password_hash.utils.compat import PY3, JYTHON
import warnings
...
import zdppy_password_hash.registry as registry
from zdppy_password_hash.tests.backports import TestCase as _TestCase, skip, skipIf, skipUnless, SkipTest
from zdppy_password_hash.utils import has_rounds_info, has_salt_info, rounds_cost_values, \
                          rng as sys_rng, getrandstr, is_ascii_safe, to_native_str, \
                          repeat_string, tick, batch
from zdppy_password_hash.utils.compat import iteritems, irange, u, unicode, PY2, nullcontext
...
class TestCase(_TestCase):
    """zdppy_password_hash-specific test case class
    ...
    * suite of methods for matching against warnings
    ...
    def require_stringprep(self):
        ...
        from zdppy_password_hash.utils import stringprep
        if not stringprep:
            from zdppy_password_hash.utils import _stringprep_missing_reason
...
class HandlerCase(TestCase):
    """base class for testing password hash handlers (esp zdppy_password_hash.utils.handlers subclasses)
    ...
    def setUp(self):
        ...
        from zdppy_password_hash.utils import handlers
    ...
    def test_13_max_salt_size(self):
        ...
        if max_size is None or max_size > (1 << 20):
            ...
            if has_relaxed_setting(handler):
                with warnings.catch_warnings(record=True): # issues zdppy_password_hashhandlerwarning
    ...
    def test_has_rounds_using_w_min_rounds(self):
        ...
        # NOTE: formerly issued a warning in zdppy_password_hash 1.6, now just a wrapper for .using()
    ...
    def test_63_large_secret(self):
        ...
        from zdppy_password_hash.exc import PasswordSizeError
        from zdppy_password_hash.utils import MAX_PASSWORD_SIZE
    ...
    def test_64_forbidden_chars(self):
        ...
        if isinstance(chars, bytes):
            from zdppy_password_hash.utils.compat import iter_byte_chars
    ...
    def assert_is_masked(self, value):
        """
        check value properly masked by :func:`zdppy_password_hash.utils.mask_value`
        """
        if value is None:
            return
        self.assertIsInstance(value, unicode)
    ...
    def test_77_fuzz_input(self, threaded=False):
        ...
        * runs output against other "external" verifiers such as OS crypt()
        ...
        :param report_thread_state:
            if true, writes state of loop to current_thread().zdppy_password_hash_fuzz_state.
        ...
        # gather info
        from zdppy_password_hash.utils import tick
...
class OsCryptMixin(HandlerCase):
    ...
    * if crypt support is missing, inserts fake crypt support to simulate
      a working safe_crypt, to test zdppy_password_hash's codepath as fully as possible.
    ...
    * check that native crypt support is detected correctly for known platforms.
    ...
    def _get_skip_backend_reason(cls, backend):
        ...
        from zdppy_password_hash.utils import has_crypt
    ...
    def _use_mock_crypt(self):
        """
        patch zdppy_password_hash.utils.safe_crypt() so it returns mock value for duration of test.
    ...
    def test_81_crypt_fallback(self):
        ...
        if self.has_os_crypt_fallback:
            ...
            from zdppy_password_hash.exc import InternalBackendError as err_type
            hash = self.get_sample_hash()[1]
    ...
    def fuzz_verifier_crypt(self):
        ...
        from zdppy_password_hash.utils import _safe_crypt_lock
