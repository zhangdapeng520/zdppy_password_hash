...
from zdppy_password_hash.context import CryptContext, CryptPolicy, LazyCryptContext
from zdppy_password_hash.utils import to_bytes, to_unicode
import zdppy_password_hash.utils.handlers as uh
from zdppy_password_hash.tests.utils import TestCase, set_file
...
class CryptPolicyTest(TestCase):
    ...
    #---------------------------------------------------------------
    # NOTE: copy of this is stored in file zdppy_password_hash/tests/sample_config_1s.cfg
    sample_config_1s = """\
[zdppy_password_hash]
...
"""
    ...
    if not os.path.exists(sample_config_1s_path) and resource_filename:
        ...
        sample_config_1s_path = resource_filename("zdppy_password_hash.tests",
                                                  "sample_config_1s.cfg")

    # make sure sample_config_1s uses \n linesep - tests rely on this
    assert sample_config_1s.startswith("[zdppy_password_hash]\nschemes")
    ...
    sample_config_1prd = dict(
        ...
        default = "md5_crypt", # NOTE: zdppy_password_hash <= 1.5 was handler obj.
        # NOTE: not maintaining backwards compat for rendering to "10%"
        all__vary_rounds = 0.1,
        bsdi_crypt__max_rounds = 30000,
        bsdi_crypt__default_rounds = 25000,
